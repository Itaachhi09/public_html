...
<div class="page-container">
  ...
  <div class="page-header">
    ...
    <div class="header-actions">
      <button onclick="openEventModal(); return false;">+ Log Event</button>
    </div>
  </div>
        ...
        <tbody>
          ...
          <tr class="event-row" data-status="<?php echo strtolower($e['status']); ?>">
            ...
            <td class="action-buttons">
              ...
              <button onclick="approveEvent(<?php echo (int)$e['id']; ?>, event)" class="action-btn approve" title="Approve">✓ Approve</button>
              <button onclick="rejectEvent(<?php echo (int)$e['id']; ?>, event)" class="action-btn reject" title="Reject">✕ Reject</button>
              <?php else: ?>
  ...
  <!-- Log Event Modal -->
<div id="eventModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 99999; justify-content: center; align-items: flex-start; overflow-y: auto; padding-top: 20px; padding-bottom: 20px;">
    <div style="background: white; border-radius: 8px; max-width: 700px; width: 95%; box-shadow: 0 20px 25px rgba(0,0,0,0.15); position: relative; z-index: 100000;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 2rem; border-bottom: 2px solid #e5e7eb; position: sticky; top: 0; background: white; z-index: 100001;">
            <h2 style="font-size: 1.5rem; margin: 0; color: #1f2937;">+ Log Event</h2>
            <button onclick="closeEventModal()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; color: #6b7280; flex-shrink: 0; z-index: 100002;">✕</button>
        </div>

        <!-- Modal Body -->
        <div style="padding: 2rem; overflow-y: auto; max-height: calc(100vh - 200px); position: relative; z-index: 100000;">
            <form id="eventForm" method="post" action="<?php echo htmlspecialchars($handlerUrl); ?>" onsubmit="return handleEventSubmit(event)" style="position: relative; z-index: 100000;">
                <input type="hidden" name="action" value="create">

                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Employee <span class="required">*</span></label>
                        <select name="employee_id" required class="form-select" style="width: 100%; box-sizing: border-box; pointer-events: auto; z-index: 100010;">
                    ...
                    <div class="form-group">
                        ...
                        <select name="procedure_type_id" required class="form-select" style="width: 100%; box-sizing: border-box; pointer-events: auto; z-index: 100010;">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" name="duty_date" required class="form-input" style="width: 100%; box-sizing: border-box; pointer-events: auto; z-index: 100010;">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Quantity <span class="required">*</span></label>
                        <input type="number" name="quantity" required class="form-input" value="1" min="1" style="width: 100%; box-sizing: border-box; pointer-events: auto; z-index: 100010;">
                    </div>
                </div>

                <!-- Modal Footer -->
                <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
                    <button type="button" class="btn btn-sm" onclick="closeEventModal(); return false;" style="background: #e5e7eb; color: #1f2937; flex: 0 0 auto;">Cancel</button>
                    <button type="submit" class="btn btn-primary btn-sm" style="flex: 0 0 auto;">Log Event (Pending Approval)</button>
                </div>
            </form>
        </div>
    </div>
...
// Approve Event with AJAX
function approveEvent(eventId, event) {
    event.preventDefault();
...
// Reject Event with AJAX
function rejectEvent(eventId, event) {
    event.preventDefault();
