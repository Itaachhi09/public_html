    ...
    switch ($action) {
        ...
        case 'getEmployeeSalary':
            ...
            if (!$emp_id) {
                ...
                // Get employee's current salary from their contract assignment
                $dbInstance = Database::getInstance();
                $db = $dbInstance->getConnection();
                
                // First try to get from most recent salary adjustment that's approved
                $stmt = $db->prepare("
                    SELECT sa.new_amount as salary
                    FROM salary_adjustments sa
                    WHERE sa.employee_id = ? AND sa.status = 'approved'
                    ORDER BY sa.effective_date DESC, sa.approved_at DESC
                    LIMIT 1
                ");
                $stmt->execute([$emp_id]);
                ...
                if ($result && $result['salary']) {
                    $response = ['success' => true, 'salary' => $result['salary']];
                } else {
                    // Fallback: try to get from employee contract or base salary
                    $stmt = $db->prepare("
                    ...
                    if ($employee && $employee['base_salary']) {
                        $response = ['success' => true, 'salary' => $employee['base_salary']];
                    } else {
                        $response = ['success' => true, 'salary' => 0];
                    }
                }
            }
            break;
