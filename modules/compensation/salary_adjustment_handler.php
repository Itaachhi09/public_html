...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$employee_id || !$adjustment_type || !$trigger_type) {
            $err = 'Employee, adjustment type, and trigger type are required.';
        } elseif (!in_array($adjustment_type, ['Temporary', 'Permanent'], true)) {
            $err = 'Adjustment type must be Temporary or Permanent.';
        } elseif (!in_array($trigger_type, ['Promotion', 'Approved exception', 'Policy change'], true)) {
            $err = 'Trigger type must be Promotion, Approved exception, or Policy change.';
